...
class LossHistory():
    ...
    def append_accuracy(self, epoch, accuracy, val_accuracy):
        if not os.path.exists(self.log_dir):
            os.makedirs(self.log_dir)
    ...
    def accuracy_plot(self):
        iters = range(len(self.accuracys))

        plt.figure()
        plt.plot(iters, self.accuracys, 'red', linewidth = 2, label='train accuracy')
        plt.plot(iters, self.val_accuracy, 'coral', linewidth = 2, label='val accuracy')
